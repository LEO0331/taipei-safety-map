...
import type { BicycleTheftSummary, IncidentTimeOfDayCategory } from '../src/types.ts';
import { convertBicycleTheftRow, readCsv, writeJson } from './shared.ts';

const path = process.argv[2] ?? 'data/raw/bicycle-theft-records/bicycle-theft-records.csv';
const rows = await readCsv(path);
const records = rows.map(convertBicycleTheftRow).sort((a, b) => (a.date ?? '').localeCompare(b.date ?? ''));
...
const summary: BicycleTheftSummary = {
...
await writeJson('public/data/bicycle-theft-records.json', records);
await writeJson('public/data/bicycle-theft-summary.json', summary);
await writeJson('public/data/bicycle-theft-location-buckets.json', buckets.sort((a, b) => b.recordCount - a.recordCount));
await writeJson('public/data/bicycle-theft-conversion.json', {
...
console.log(`Converted ${records.length} bicycle theft rows.`);
