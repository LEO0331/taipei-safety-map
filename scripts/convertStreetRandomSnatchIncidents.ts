...
import type { IncidentTimeOfDayCategory, StreetRandomSnatchIncidentSummary } from '../src/types.ts';
import { convertStreetRandomSnatchIncidentRow, readCsv, writeJson } from './shared.ts';

const path = process.argv[2] ?? 'data/raw/street-random-snatch-incidents/street-random-snatch-incidents.csv';
const rows = await readCsv(path);
const records = rows.map(convertStreetRandomSnatchIncidentRow).sort((a, b) => (a.date ?? '').localeCompare(b.date ?? ''));
...
const summary: StreetRandomSnatchIncidentSummary = {
  ...
  recordsWithValidTimePeriod: records.filter((record) => record.timePeriodValid).length,
  recordsWithInvalidTimePeriod: records.filter((record) => !record.timePeriodValid).length,
  recordsWithNotes: records.filter((record) => record.note).length,
...
await writeJson('public/data/street-random-snatch-incidents.json', records);
await writeJson('public/data/street-random-snatch-incident-summary.json', summary);
await writeJson('public/data/street-random-snatch-incident-conversion.json', {
  ...
  timeBandParseWarnings: records.filter((record) => !record.timePeriodValid).map((record) => record.incidentTimeBandRaw ?? record.id),
  locationParseWarnings: records.filter((record) => !record.district).slice(0, 10).map((record) => record.incidentLocationRaw ?? record.id),
  unexpectedCaseTypeWarnings: records.filter((record) => record.caseType !== 'street_random_snatch').map((record) => record.caseTypeRaw ?? record.id),
  duplicateExamples,
});

console.log(`Converted ${records.length} street random snatch rows.`);
