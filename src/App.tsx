...
import KindergartenChildTransportVehicles from './KindergartenChildTransportVehicles';
import SoilWaterConservationViolations from './SoilWaterConservationViolations';
import AlcoholTobaccoBusinessInspections from './AlcoholTobaccoBusinessInspections';
...
import ElectricalInspectionMaintenanceBusinesses from './ElectricalInspectionMaintenanceBusinesses';
import type {
  AedLocation,
  AirRaidShelter,
...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="main-navigation" aria-label={language === 'zh' ? '主要導覽' : 'Main navigation'}>
        {(() => {
          const groups: Array<{ id: NavigationGroup; label: string; description: string; items: Array<[Tab, string]> }> = [
            ...
            { id: 'health', label: language === 'zh' ? '健康與設施' : 'Health & facilities', description: language === 'zh' ? '健康、消防、場所與公用設施資料' : 'Health, fire, premises, and public-facility data', items: [['health', t.publicHealth], ['tobaccoControl', language === 'zh' ? '菸害防制稽查' : 'Tobacco Inspections'], ['businessHygieneViolationCases', language === 'zh' ? '營業衛生違規（歷史）' : 'Business Hygiene Violations (Historical)'], ['alcoholTobaccoBusinessInspections', language === 'zh' ? '菸酒業者抽檢' : 'Tobacco & Alcohol Inspections'], ['fireSafetyDeclarations', language === 'zh' ? '消防檢修申報' : 'Fire Safety Declarations'], ['lpgFacilities', language === 'zh' ? '液化石油氣場所' : 'LPG Facilities'], ['electricalBusinesses', language === 'zh' ? '用電設備檢驗維護業' : 'Electrical Inspection Businesses'], ['fireDonations', language === 'zh' ? '消防捐贈實物' : 'Fire Dept Donations']] },
      ...
      {activeTab === 'kindergartenChildTransportVehicles' && <KindergartenChildTransportVehicles language={language} />}
      {activeTab === 'soilWaterConservationViolations' && <SoilWaterConservationViolations language={language} />}
      {activeTab === 'alcoholTobaccoBusinessInspections' && <AlcoholTobaccoBusinessInspections language={language} />}
      ...
      {activeTab === 'electricalBusinesses' && <ElectricalInspectionMaintenanceBusinesses records={data.electricalInspectionMaintenanceBusinesses} language={language} />}
